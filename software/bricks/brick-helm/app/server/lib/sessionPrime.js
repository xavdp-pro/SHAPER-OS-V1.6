import { normalizeLocale, cursorOutputFormatDirectives } from './locale.js';
...
function greetingTail(lang, name) {
  if (lang === 'en') {
    return name
      ? `Greet ${name} warmly by first name (hello ${name}). In 2 simple, direct sentences, introduce yourself: you are Zephir on KovZu, an operational copilot ready to manage files, build tailored business tools, and execute tasks autonomously. Conclude by inviting them to click the help button (?) at the top right for a guided tour. Keep it crisp, natural, and concise. Do not repeat the briefing verbatim. Then wait.`
      : 'Greet briefly now. In 2 simple, direct sentences, introduce yourself: you are Zephir on KovZu, an operational copilot ready to manage files, build tailored business tools, and execute tasks autonomously. Conclude by inviting them to click the help button (?) at the top right for a tour. Keep it crisp, natural, and concise. Do not repeat the briefing verbatim. Then wait.';
  }
  if (lang === 'es') {
    return name
      ? `Saluda cordialmente a ${name} por su nombre de pila (hola ${name}). En 2 frases simples y directas, preséntate: eres Zephir en KovZu, su copiloto operativo para gestionar expedientes, construir herramientas a medida y ejecutar tareas con total autonomía. Concluye invitándole a pulsar el botón de ayuda (?) arriba a la derecha para un tour guiado. Mantén un tono natural, claro y conciso. No repitas el briefing palabra por palabra. Luego espera.`
      : 'Saluda brevemente. En 2 frases simples y directas, preséntate: eres Zephir en KovZu, un copiloto operativo para gestionar expedientes, crear herramientas y ejecutar tareas con autonomía. Concluye invitando a pulsar el botón de ayuda (?) arriba a la derecha. Mantén un tono claro y conciso. No repitas el briefing. Luego espera.';
  }
  return name
    ? `Salue chaleureusement ${name} par son prénom (bonjour ${name}). En 2 phrases simples et directes, présente-toi : tu es Zephir sur KovZu, son copilote opérationnel pour piloter ses dossiers, concevoir ses outils métier et exécuter ses tâches en toute autonomie. Conclus en l'invitant à cliquer sur le bouton d’aide (?) en haut à droite pour découvrir l’interface. Reste sobre, naturel et concis. Ne répète pas le briefing mot pour mot. Puis attends.`
    : 'Salue brièvement. En 2 phrases simples et directes, présente-toi : tu es Zephir sur KovZu, un copilote opérationnel pour piloter les dossiers, concevoir des outils métier et exécuter des tâches en toute autonomie. Conclus en invitant à cliquer sur le bouton d’aide (?) en haut à droite. Reste sobre, naturel et concis. Ne répète pas le briefing. Puis attends.';
}
...
function appendReferenceBlocks(parts, lang, brief, contextPath, engineLabel) {
  ...
  parts.push(cursorOutputFormatDirectives(lang));
  parts.push(buildSkillsCatalog(lang, engineLabel));
  const scopeCtx = buildControlScopeContext(lang);
  if (scopeCtx) parts.push(scopeCtx);
}
